...
import { Network, BookOpen, Brain, ChevronLeft, Loader2, FlaskConical } from "lucide-react";
      ...
      >
        ...
        >
          {[
            ...
            { path: `/project/${projectId}/sandbox`, label: "Sandbox", icon: FlaskConical },
          ].map((tab) => {
            const Icon = tab.icon;
            const isActive = location.pathname === tab.path ||
              (tab.path.endsWith("/map") && !location.pathname.endsWith("/guide") && !location.pathname.endsWith("/quiz") && !location.pathname.endsWith("/sandbox"));
            return (
              <Link
                key={tab.path}
                to={tab.path}
